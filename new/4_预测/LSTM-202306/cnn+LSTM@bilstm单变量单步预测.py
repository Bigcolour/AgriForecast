...
# %%
data=pd.read_excel('分解数据\data_3228.xlsx',usecols=[0])
...
# %%
model.fit(x=seq_test_x_1[:2358],y=seq_test_y[:2358],batch_size=16,epochs=50,verbose=2)#模型训练。一共1148个数据，拿前918个做训练
...
plt.plot(yre[:2358],'b-',label='验证')
plt.plot([2358+i for i in range(len(yre[2358:]))],yre[2358:],'g-',label='预测')
...
# print(yre[2358:], file=f)
import pandas as pd

df_Mode_ = pd.DataFrame(yre[2358:])
...
print(r2_score(origin[2358:-look_back],yre[2358:]))

# %%
from sklearn.metrics import mean_squared_error,mean_absolute_error

# %%
print(mean_absolute_error(origin[2358:-look_back],yre[2358:]))#mae
print(mean_squared_error(origin[2358:-look_back],yre[2358:]))#mse

# %%
print('rmse:',mean_squared_error(origin[2358:-look_back],yre[2358:])**0.5)#rmse
