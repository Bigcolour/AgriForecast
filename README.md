...
- Naive Forecast（朴素预测）：使用最后一个观测值作为未来预测。
- Moving Average（移动平均）：使用最近 `window` 个观测值的均值预测未来。
- Simple Exponential Smoothing（简单指数平滑）：使用 `alpha` 控制近期数据权重。
- Linear Trend（线性趋势）：拟合一条线性趋势并向未来延伸。
